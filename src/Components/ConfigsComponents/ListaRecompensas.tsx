...
function ListaRecompensas() {
    ...
    const {tabelaRecompensas,setTabelaRecompensa} = useContext(TabelasContext)

    const handleDelete = (tarefa:Elemento)=>{
        tabelaRecompensas.deleteItem(tarefa);
        setTabelaRecompensa(tabelaRecompensas)
    }

    const listaTarefas = tabelaRecompensas.listItems().map((t,index)=>{
        return(
